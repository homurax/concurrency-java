...
import com.homurax.commonFriends.concurrent.mapper.CommonPersonMapper;
...
public class ConcurrentSocialNetwork {
    ...
    public static List<PersonPair> bidirectionalCommonContacts(
        ...
        List<PersonPair> peopleCommonContacts = group.entrySet()
                .parallelStream()
                .map(entry -> {
                    Collection<String> commonContacts = entry.getValue().parallelStream().map(Person::getContacts).collect(intersecting);
