package com.homurax.structures.hash;

import com.homurax.structures.hash.data.ConcurrentLoaderAccumulator;
import com.homurax.structures.hash.data.ExtendedProduct;
import com.homurax.structures.hash.data.Product;
import com.homurax.structures.hash.data.Review;
...
public class ConcurrentHashMainAmazon {
    ...
    public static void main(String[] args) {
        ...
        try {
            ConcurrentLinkedDeque<Product> productList = Files
            ...
            ConcurrentHashMap<String, List<ExtendedProduct>> productsByBuyer = (ConcurrentHashMap<String, List<ExtendedProduct>>) productList
                    ...
                    .parallel()
                    .collect(Collectors.groupingByConcurrent(ExtendedProduct::getBuyer));
            ...
            productsByBuyer.forEach((id, list) -> {
                double average = list.stream().mapToDouble(ExtendedProduct::getValue).average().getAsDouble();
            ...
            // Transformer
            BiFunction<String, List<ExtendedProduct>, List<ExtendedProduct>> transformer = (key, value) -> value.stream()
            ...
            badReviews.forEach(product -> System.out.println(product.getTitle() + ":" + product.getBuyer() + ":" + product.getValue()));

            // Compute method
            ConcurrentHashMap<String, LongAdder> counter = new ConcurrentHashMap<>();

            badReviews.forEach(product -> counter.computeIfAbsent(product.getTitle(), title -> new LongAdder()).increment());

            counter.forEach((title, count) -> System.out.println(title + ":" + count));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
